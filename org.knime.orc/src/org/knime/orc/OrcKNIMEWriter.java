 ...
 *
 * History
 *   23.04.2018 ("Mareike Höger, KNIME"): created
 */
...
/**
 * ORC Writer for KNIME {@link DataRow}. It holds the rows in batches and hands them to the ORC writer once the maximum
 * batch size is reached.
 *
 * @author Mareike Hoeger, KNIME GmbH, Konstanz, Germany
 */
public class OrcKNIMEWriter {
     ...
     *
     * @param binFile the target file
     * @param spec the data table spec
     * @param isWriteRowKey whether the key should be written
     * @param stripSize size of the strip
     * @param batchSize size of the batch
     * @param compression the compression to use for writing
     * @throws IOException if writer cannot be initialized
     */
    public OrcKNIMEWriter(final RemoteFile<Connection> binFile, final DataTableSpec spec, final boolean isWriteRowKey,
        final long stripSize, final int batchSize, final String compression) throws IOException {
        m_writeRowKey = isWriteRowKey;
        m_targetFile = binFile;
    ...
    private void initWriter() throws IOException {
        ...
        for (int i = 0; i < m_tableSpec.getNumColumns(); i++) {
            DataColumnSpec colSpec = m_tableSpec.getColumnSpec(i);
            String name = colSpec.getName().replaceAll("[^a-zA-Z]", "-");
        ...
        m_writer = OrcFile.createWriter(new Path(m_targetFile.getURI()), orcConf);
        m_rowBatch = schema.createRowBatch(m_batchSize);
    }
    ...
    public void writeRow(final DataRow row) throws IOException {
        ...
        for (; c < m_rowBatch.numCols; c++) {
            DataCell cell = row.getCell(m_writeRowKey ? c - 1 : c);
        ...
        if (m_rowBatch.size == m_batchSize - 1) {
    ...
    public void writeMetaInfoAfterWrite(final NodeSettingsWO settings) {
        ...
        for (Pair<String, OrcType<?>> entry : m_fields) {
            ...
            m_writer.addUserMetadata("knime_tablename.",
                ByteBuffer.wrap(m_tableSpec.getName().getBytes(Charset.forName("UTF-8"))));
            for (String column : m_tableSpec.getColumnNames()) {
                DataColumnSpec columnSpec = m_tableSpec.getColumnSpec(column);

                Class<? extends DataCell> cellClass = columnSpec.getType().getCellClass();
